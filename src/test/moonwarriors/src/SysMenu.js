...
var SysMenu = cc.Layer.extend({
    ...
    onNewGame:function (pSender) {
        ...
        new cc.plugin.loader.Loader().
            addResources(g_maingame).
            startLoading( function onEnd( resources ) {
    ...
    update:function () {
        if (this._ship.y > 750) {
            ...
            this._ship.runAction(cc.moveBy(
                parseInt(2 + 4 * Math.random(), 10),
                cc.p(Math.random() * winSize.width * (Math.random() <.5?1:-1), this._ship.y + 750)
            ));
        }
    },
